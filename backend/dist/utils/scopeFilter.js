...
exports.buildPayrollWhereClause = buildPayrollWhereClause;
const prisma_1 = require("../prisma");
async function buildEmployeeWhereClause(user) {
    // SUPER_ADMIN / SYSTEM_ADMIN / HR_DIRECTOR → ไม่กรอง
    if (user.level >= 80)
        return {};
    const scope = await prisma_1.prisma.dataScope.findUnique({
...
async function buildPayrollWhereClause(user) {
    if (user.level >= 80)
        return {};
    const scope = await prisma_1.prisma.payrollScope.findUnique({
        where: { userId: user.id }
    });
    if (!scope) {
        // EMPLOYEE -> only self
