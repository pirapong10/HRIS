...
exports.approveRequest = exports.createRequest = exports.getRequests = void 0;
const prisma_1 = require("../prisma");
const getRequests = async (req, res) => {
    try {
        const reqs = await prisma_1.prisma.approvalRequest.findMany({
...
const createRequest = async (req, res) => {
    try {
        ...
        const request = await prisma_1.prisma.approvalRequest.create({
            data: {
                type,
                referenceId,
                requesterId: empId,
...
const approveRequest = async (req, res) => {
    try {
        ...
        const request = await prisma_1.prisma.approvalRequest.findUnique({
            where: { id: Number(id) },
            include: { requester: true }
        });
        if (!request)
        ...
        if (request.type === 'OT' && action === 'APPROVED') {
            const ot = await prisma_1.prisma.oT.findUnique({ where: { id: request.referenceId } });
            if (ot) {
                ...
                const existingOts = await prisma_1.prisma.oT.findMany({
                    where: {
                        empId: ot.empId,
                        status: 'approved',
